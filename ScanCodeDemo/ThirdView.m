...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        label_text.text = @"将二维码放入矩形框内,即可自动扫描";
...
- (void)layoutSubviews{
    ...
    label_text.center = CGPointMake(self.bounds.size.width * 0.5, CGRectGetMaxY([self clearDrawRect])+40);
}

- (void)initQRLine{
    qrLine = [[UIImageView alloc]initWithFrame:CGRectMake(LCDW/2 - self.transparentArea.width/2, LCDH/2 - self.transparentArea.height/2, self.transparentArea.width, 8)];
    qrLine.image = [UIImage imageNamed:@"QRCodeScanLine@2x.png"];
...
- (CGRect)clearDrawRect{
    //中间清空的矩形框
    CGRect clearDrawRect = CGRectMake(LCDW/2 - self.transparentArea.width/2, LCDH/2 - self.transparentArea.height/2, self.transparentArea.width, self.transparentArea.width);
    return clearDrawRect;
    
}

- (void)show{
    CGRect rect = [self clearDrawRect];
    if (upOrDown == NO) {
        num ++;
        qrLine.frame = CGRectMake(CGRectGetMinX(rect), CGRectGetMinY(rect) + num, rect.size.width, 8);
        if (num == CGRectGetHeight(rect)) {
            upOrDown = YES;
        }
    }else{
        num --;
